                        ...
                        <div class="rounded-xl overflow-hidden bg-white shadow-sm" style="border: 1px solid #E8E0CC;">
                            ...
                            <div class="p-4">
                                ...
                                <div class="flex items-end justify-between mt-2">
                                    ...
                                    @if($isEnquiry)
                                        {{-- Custom-quote items still funnel to the contact form --}}
                                        <a href="{{ $prodUrl }}" class="px-4 py-2 rounded text-xs text-white font-medium" style="background: #0D3D1F;">Enquire</a>
                                    @else
                                        @php $added = !empty($justAdded[$prod->id]); @endphp
                                        <button wire:click="addToCart({{ $prod->id }})"
                                                wire:loading.attr="disabled"
                                                wire:target="addToCart({{ $prod->id }})"
                                                class="inline-flex items-center gap-1.5 px-4 py-2 rounded text-xs font-medium transition-colors"
                                                style="background: {{ $added ? '#1B5E20' : '#0D3D1F' }}; color: white;">
                                            @if($added)
