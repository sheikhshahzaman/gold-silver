<div class="max-w-2xl mx-auto px-4 py-6">
    ...
    <div class="mb-6">
        <div class="flex items-center justify-between mb-2">
            <h1 class="text-2xl md:text-3xl font-bold" style="color: #0A2E23;">Buy</h1>
    ...
    <div class="flex items-center justify-center mb-8">
        <div class="flex items-center gap-0">
            <div class="flex items-center justify-center w-10 h-10 rounded-full font-bold text-sm transition-all duration-300
                {{ $step >= 1 ? 'bg-gold text-white' : 'text-gray-400' }}" @if($step < 1) style="background: #E8DFD0;" @endif>
                1
            </div>
            <div class="w-20 h-0.5 transition-all duration-300
                {{ $step >= 2 ? 'bg-gold' : '' }}" @if($step < 2) style="background: #E8DFD0;" @endif></div>
            <div class="flex items-center justify-center w-10 h-10 rounded-full font-bold text-sm transition-all duration-300
                {{ $step >= 2 ? 'bg-gold text-white' : 'text-gray-400' }}" @if($step < 2) style="background: #E8DFD0;" @endif>
                2
            </div>
        </div>
    </div>
        ...
        <div class="space-y-6">
            ...
            <div class="grid grid-cols-2 gap-4">
                <button wire:click="selectMetal('gold')"
                    class="glass-card p-6 text-center transition-all duration-300 cursor-pointer
                        {{ $selectedMetal === 'gold' ? '!border-gold' : '' }}">
                ...
                <button wire:click="selectMetal('silver')"
                    class="glass-card p-6 text-center transition-all duration-300 cursor-pointer
                        {{ $selectedMetal === 'silver' ? '!border-gold' : '' }}">
                    <div class="flex flex-col items-center gap-3">
    ...
    @else
        <div class="space-y-6">
            <div class="text-center mb-6">
                ...
                <p class="text-sm" style="color: #888;">See BUY price instantly</p>
            </div>
            ...
            <div class="space-y-3">
                ...
                <div class="flex items-center gap-3">
                    <button wire:click="decrementQuantity"
                        class="w-12 h-12 rounded-xl transition-colors flex items-center justify-center" style="background: #F0E8DB; color: #0A2E23;">
                    ...
                    <input type="number" wire:model.live.debounce.300ms="quantity"
                        min="1" max="9999"
                        class="flex-1 rounded-xl px-4 py-3 text-center text-xl font-bold focus:outline-none focus:ring-2 focus:ring-gold/30 transition-colors [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
            ...
            <div class="glass-card p-6 text-center">
                <p class="text-sm mb-2" style="color: #888;">Total Buy Price</p>
                @if($calculatedPrice !== null)
                    <p class="text-3xl md:text-4xl font-bold text-gold">
